...
<%-- <a href=<%= "\"StudentSemPerformance.jsp?rno=" + "WEEKS" + "\"" %>  id="semMarks">SemMarks</a> --%>
...
<%
 ...
 for(StudentPercent str: marksList){
	%>
		<tr><td><%= str.getRno() %></td><td><%= str.getName()%> 
		</td><td><%= str.getBranch() %></td><td><%= str.getSem1percent() %></td><td><%= str.getSem2percent() %></td>
		<td><%= str.getSem3percent() %></td><td><%= str.getFinalpercent() %></td>
		</tr>
